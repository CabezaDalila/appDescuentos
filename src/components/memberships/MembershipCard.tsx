...
const MembershipCard: React.FC<MembershipCardProps> = ({
  ...
  const cardClasses = `
    ...
    ${variant === "carousel" ? "w-64 h-32 flex-shrink-0" : "w-full h-24"}
        ...
        <CardContent className="relative z-10 px-8 py-4 pt-8 flex flex-col gap-y-4">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="font-semibold text-lg text-white/90 leading-tight">
                  {membership.name}
                </h3>
                <p className="text-xs text-white/70 capitalize mt-0.5 tracking-wide">
                  Banco
                </p>
              </div>
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-white/80 capitalize mt-0.5 tracking-wide">
                {membership.category === "club"
                  ? "Club de Beneficios"
                  : membership.category.charAt(0).toUpperCase() +
                    membership.category.slice(1)}
              </p>
            </div>
          </div>
          {showStatus && (
