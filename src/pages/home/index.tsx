    ...
    <div className="w-full min-h-screen bg-white">
      ...
      <div className="w-full px-3 sm:px-4 pt-2 sm:pt-4 pb-3 sm:pb-4">
        <div className="flex items-start justify-between mb-4 sm:mb-5">
          <div className="flex-1 min-w-0 pr-2 sm:pr-3">
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 mb-1 sm:mb-2">{greeting}</h1>
            <p className="text-xs sm:text-sm text-gray-600 leading-relaxed">Descubre ofertas increíbles cerca de ti</p>
          </div>
          ...
          {showSearchResults && (
            <div className="absolute top-full left-0 right-0 mt-1 sm:mt-2 bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-48 sm:max-h-64 overflow-y-auto">
              {isSearching ? (
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex-1 min-w-0">
                          <div className="text-xs sm:text-sm font-medium text-gray-900 truncate">{discount.name || discount.title}</div>
                          {discount.description && (
                            <div className="text-xs text-gray-500 line-clamp-1 mt-0.5">{discount.description}</div>
                          )}
                        </div>
                ...
                <div className="p-3 sm:p-4 text-center text-gray-500">
                  <span className="text-xs sm:text-sm">No se encontraron resultados para "{searchTerm}"</span>
                </div>
              ) : null}
            </div>
          )}
      ...
      <div className="w-full px-3 sm:px-4 mb-4 sm:mb-5">
        <div className="flex justify-between items-center mb-2 sm:mb-3">
          <h2 className="text-sm sm:text-base font-semibold text-gray-900">Categorías</h2>
        </div>
        <div className="grid grid-cols-4 gap-2 sm:gap-3">
          {categories.map((category) => (
            ...
            >
              <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-full ${category.color} flex items-center justify-center shadow-lg hover:scale-105 transition-transform`}>
                <category.icon className={`w-5 h-5 sm:w-6 sm:h-6 ${category.iconColor}`} />
              </div>
              <span className="text-[10px] sm:text-xs font-medium text-gray-700 text-center leading-tight">{category.label}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="w-full px-3 sm:px-4 mb-4 sm:mb-5">
        <div className="flex justify-between items-center mb-2 sm:mb-3">
          <h2 className="text-sm sm:text-base font-semibold text-gray-900">Explorar categorías</h2>
        ...
        <div className="grid grid-cols-2 gap-2 sm:gap-3">
          {exploreCategories.map((category) => (
            ...
            >
              ...
              <div className="text-xs sm:text-sm font-semibold">{category.label}</div>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="w-full px-3 sm:px-4 mb-4 sm:mb-5">
        <div className="flex justify-between items-center mb-2 sm:mb-3">
          <h2 className="text-sm sm:text-base font-semibold text-gray-900">Bancos populares</h2>
          <button className="flex items-center gap-1 text-purple-600 text-xs font-medium hover:text-purple-700">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          {popularBanks.map((bank) => (
            ...
            >
              ...
              <div className="text-[10px] sm:text-xs font-medium text-gray-700 text-center leading-tight">{bank.name}</div>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="w-full px-3 sm:px-4 mb-4 sm:mb-5">
        <div className="flex justify-between items-center mb-2 sm:mb-3">
          <div>
            <h2 className="text-sm sm:text-base font-semibold text-gray-900">Hecho para ti</h2>
            <p className="text-[10px] sm:text-xs text-gray-600">Basado en tus preferencias</p>
          </div>
          <button className="flex items-center gap-1 text-purple-600 text-xs font-medium hover:text-purple-700">
          ...
          {personalizedOffers.map((offer) => (
            <Card key={offer.id} className="overflow-hidden hover:shadow-lg transition-shadow">
              <CardContent className="p-0">
                <div className={`${offer.color} p-2.5 sm:p-3 text-white relative`}>
                  ...
                  <div className="text-center">
                    <div className="text-xl sm:text-2xl font-bold mb-0.5 sm:mb-1">{offer.discount}</div>
                    <div className="text-[10px] sm:text-xs opacity-90">{offer.category}</div>
                  </div>
                </div>
                <div className="p-2.5 sm:p-3">
                  <div className="text-xs sm:text-sm font-medium text-gray-900 mb-0.5 sm:mb-1">{offer.title}</div>
                  <div className="text-[10px] sm:text-xs text-gray-600">{offer.subtitle}</div>
                </div>
              </CardContent>
            </Card>
          ))}
      ...
      <div className="w-full px-3 sm:px-4 mb-4 sm:mb-5">
        <div className="flex justify-between items-center mb-2 sm:mb-3">
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <h2 className="text-sm sm:text-base font-semibold text-gray-900">Tendencias</h2>
          </div>
        ...
        <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-2 scrollbar-hide">
          {trendingOffers.map((offer) => (
            <Card key={offer.id} className="flex-shrink-0 w-40 sm:w-48 hover:shadow-lg transition-shadow">
              <CardContent className="p-0">
                ...
                <div className="p-2 sm:p-3">
                  <div className="flex justify-between items-center mb-1.5 sm:mb-2">
                    ...
                    <div className="flex items-center gap-0.5 sm:gap-1">
                      ...
                      <span className="text-[9px] sm:text-xs text-gray-600">{offer.rating}</span>
                    </div>
                  </div>
                  <div className="text-xs sm:text-sm font-medium text-gray-900 leading-tight">{offer.title}</div>
                </div>
              </CardContent>
            </Card>
          ))}
      ...
      <div className="w-full px-3 sm:px-4 mb-4 sm:mb-6 pb-20 sm:pb-6">
        <div className="flex justify-between items-center mb-2 sm:mb-3">
          <h2 className="text-sm sm:text-base font-semibold text-gray-900">Promociones destacadas</h2>
          <button className="flex items-center gap-1 text-purple-600 text-xs font-medium hover:text-purple-700">
            Ver todas <ArrowRight className="w-3 h-3" />
          </button>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3">
          {loading ? (
            <div className="col-span-full text-center py-6 sm:py-8">
              ...
              <p className="text-sm sm:text-base text-gray-600">Cargando descuentos...</p>
            </div>
          ) : discounts.length === 0 ? (
            <div className="col-span-full text-center py-6 sm:py-8">
              <p className="text-sm sm:text-base text-gray-600">No hay descuentos disponibles</p>
